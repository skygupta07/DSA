...
class Solution {    // using mergeSort
public:
    ...
    void mergeSort(vector<int> &count, vector<pair<int, int> > &v, int left, int right) {
        if (left >= right) 
            return; // Base case: Jab ek hi element bache toh return kar do
    ...
    vector<int> countSmaller(vector<int>& nums) {
        ...
        vector<pair<int, int> > v(N); // Pair bana rahe hain taaki original index store rahe
        for (int i = 0; i < N; i++)   
            v[i] = make_pair(nums[i], i); // (value, index) pair store kiya
        ...
        return count; // Final count array return kar diya
    }
};
