...
2 tweet ids -> [6, 5]. Tweet id 6 should precede tweet id 5 because it is posted after tweet id 5.
...
public:
    ...
    vector<int> getNewsFeed(int userId) {
        // Min heap to get top 10 recent tweets
        ...
        if (following.count(userId)) {
            for (int followeeId : following[userId]) {
                if (followeeId == userId) continue; // Avoid duplication
