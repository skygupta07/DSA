...
from applicants in real-time. This helps to determine cut-off marks for interviews 
...
public:
    ...
    // kth largest find karna tha isliye minheap declare kari...
    ...
    KthLargest(int k, vector<int>& nums) {
        ...
        // kucch log pehle pure pq ko fill karte then while (!pq.empty() && k--) wala karte ...
        // but legend people always dynamically maintain the pq of length k only ... as soon as 
        // its size increased ... simply pop the element ...

        for (int num : nums) {
            pq.push(num);

            // very interesting pattern or chalaki to remember...
            if (pq.size() > k) {
                pq.pop();
            }
        }
