...
In the first operation, we remove elements 1 and 2, then add 1 * 2 + 2 to nums. 
...
class Solution {
    public:
        int minOperations(vector<int>& nums, int k) {
            ...
            // should have atleast two elements..
    
            while (pq.size() > 1 && pq.top() < k){
                long long x = pq.top(); pq.pop();
                long long y = pq.top(); pq.pop();
    
                pq.push(2*x + y);    // 2*x + y ==== (min(x, y) * 2 + max(x, y)); 
    ...
    if already greater than or equal to k , then no ops++
