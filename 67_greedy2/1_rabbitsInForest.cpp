...
agar maan le forest mai 4 rabbit h actually 

now aapne har rabbit se puccha to wo answer de rahe h 3 3 3 3
matlab each rabbit kehna chahta h ki arey bhai mai to hu hi ... 
but mere jaise 3 aur rabbit h ...
...
public:
    ...
    int numRabbits(vector<int>& answers) {
        ...
        for (const auto &[x, count] : freq) {
            ...
            // agar answer 0 hai toh har rabbit unique hai
            if (x == 0) total += count; 
...
public:
    ...
    int numRabbits(vector <int> &answers) {
        ...
        unordered_map <int, int> mp; // key = color, value = count of rabbits with this color
...
- When a group is complete (i.e., the number of rabbits matches the group size), 
