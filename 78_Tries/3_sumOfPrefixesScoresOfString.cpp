...
struct Trie {
    ...
    void add(string &s, int i) {
        if (i) score++;  // Increment the score if we're past the root node
        if (i == s.size()) return;  // If we've reached the end of the word, stop
    ...
    int dfs(string &s, int i) {
        if (i == s.size()) return score;  // If we've reached the end of the word, return the score of the current node
