...
public:
    int maxScore(string s) {
        ...
        vector<int> pref(n, 0);  // Prefix sum of zeros
        vector<int> suff(n, 0); // Suffix sum of ones

        // Prefix count of zeros
        pref[0] = (s[0] == '0') ? 1 : 0;    // my mistake of ==    better to use bracket
        ...
        for (int i = 0; i < n - 1; i++) {  // Ensure left and right substrings are non-empty
            maxi = max(maxi, pref[i] + suff[i + 1]); // think little bit why suff[i+1]
        }
