...
numbered from 0 to n - 1 and from 0 to m - 1, respectively. 
...
in the first tree and edges2[i] = [ui, vi] indicates that there is an edge between nodes ui and vi 
in the second tree.
...
We can obtain a tree of diameter 3 by connecting node 0 from the first tree with any node 
...
We can obtain a tree of diameter 5 by connecting node 0 from the first tree with node 0 
from the second tree.
...
public:
    int minimumDiameterAfterMerge(vector<vector<int>>& edges1,
        ...
        // Calculate the diameters ofa both trees
