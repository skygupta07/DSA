...
// Include the BLACKBOX code here
...
int main() {
    // Input parsing
    int N, M; // Number of buildings (nodes) and edges (connections)
    cin >> N >> M;

    // Create the graph with N nodes
    Graph g(N, true); // True for directed graph
    ...
    for (int i = 0; i < M; ++i) {
        ...
        cin >> u >> v;
        g.add_edge(u, v);
        g.add_edge(v, u); // Add bidirectional edges since jumping is mutual
    }

    // Read start and target buildings
    int start, target;
    cin >> start >> target;
    ...
    // Get the result
    int min_jumps = bfs.min_dist(target);
