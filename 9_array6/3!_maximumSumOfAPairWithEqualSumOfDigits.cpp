...
- So for each digit sum from 1 to 81, we can store the **maximum number** seen so far with that digit sum.
- When we find another number with the same digit sum, we try to pair it with the previously stored maximum.
*/
...
When another number with the same digit sum comes in, pair it with the existing largest one in that group and update the answer if this sum is greater than the previous maximum.
