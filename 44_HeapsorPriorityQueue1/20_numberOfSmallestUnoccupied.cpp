...
public:
    int smallestChair(vector<vector<int>>& times, int targetFriend) {
        ...
        int n = times.size();
        priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>> > busy; //leave time as first , index as second
        ...
        for(int i=0;i<n;++i){
            ...
            // agar leave time <= arrival hai to process karo...
            while(!busy.empty() && busy.top().first <= arrival){
                // cout << "Chair free " << busy.top().second << endl;
                free.push(busy.top().second);   // free mai uska leave time daal diya..
                busy.pop();
            }   
