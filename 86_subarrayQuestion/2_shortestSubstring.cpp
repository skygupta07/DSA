...
In a distant galaxy, a young inventor named Leo created a robot called "Trace" 
to help him find hidden treasures. These treasures were marked on ancient maps 
stored as long strings of mysterious text. To locate a treasure, 
Trace needed to find a specific clue called the Key, made up of a unique combination of characters.
...
Trace’s job was to find the shortest section(substring) of the map that included all the characters 
of the Key, no matter the order.  If no such substring exists, Trace should return -1.
...
The shortest substring of S that contains all the characters of T is "warrmior", 
which has length 8.
...
int shortestSubstring(string s, string t) {
    ...
    for (int end = 0; end < s.size(); end++) {
        ...
        if (required.count(ch) && window[ch] == required[ch]) formed++;
        ...
        while (formed == requiredCount) {
            ...
            if (required.count(startChar) && window[startChar] < required[startChar]) formed--;
            start++;
        }
    }
